...
function camelize(str) {
    let arr = str.split('-');
    let myMap = arr.map((word, index) => index == 0 ? word : word[0].toUpperCase() + word.slice(1));
    let arr2 = myMap.join('');
    return arr2;
}
...
function getSecondsToday() {
    let today = new Date();
    return secs = today.getHours() * 3600 + today.getMinutes() * 60 + today.getSeconds();
}
...
function getSecondsToTomorrow() {
    ...
    let tommMidnight = new Date(todayDate.getFullYear(), todayDate.getMonth(), todayDate.getDate() + 1);
...
function formatDate(date) {
    ...
    if (dateDiff < SECOND) {
        ...
        str = `${date.getDate()}.${date.getMonth() + 1}.${date.getFullYear().toString().slice(-2)} ${date.getHours()}:${date.getMinutes()}`;
...
// }
console.log('Задание 12.1 - Декораторы');
